...
const char *d3d_vendor;
const char *d3d_renderer;
const char *d3d_version;
const char *d3d_extensions;
...
qboolean VID_SetWindowedMode (int modenum)
{
	...
	//ExWindowStyle = 0;
//xWindowStyle = WS_EX_TOOLWINDOW; //syq 2000.10.31
...
BOOL D3D_Init (void)
{
	...
	GUID* pDeviceGuid=NULL;

	d3d_vendor = strdup("d3d");
//	Con_Printf ("D3D_VENDOR: %s\n", d3d_vendor);
	d3d_renderer = strdup("d3d");
//	Con_Printf ("D3D_RENDERER: %s\n", d3d_renderer);

	d3d_version = strdup("v7.0");
//	Con_Printf ("D3D_VERSION: %s\n", d3d_version);
	d3d_extensions = strdup("0.1");
//	Con_Printf ("D3D_EXTENSIONS: %s\n", d3d_extensions);

	Con_Printf("Direct3D Initializing...");

    //-------------------------------------------------------------------------
	// Step 1: Create DirectDraw and set the coop level
    //-------------------------------------------------------------------------
	hr = DirectDrawCreateEx( NULL, (VOID**)&g_pDD, (REFIID)&IID_IDirectDraw7, NULL );
	if( FAILED( hr ) )
		return FALSE;
	...
	hr = IDirectDraw7_CreateSurface( g_pDD, &ddsd, &g_pddsPrimary, NULL );
	if( FAILED( hr ) )
		return FALSE;
	...
	hr = IDirectDraw7_CreateSurface( g_pDD, &ddsd, &g_pddsBackBuffer, NULL );
	//hr = g_pDD->lpVtbl->CreateSurface( &ddsd, &g_pddsBackBuffer, NULL );
	if( FAILED( hr ) )
		return hr;
	...
	hr = IDirectDraw7_CreateClipper(g_pDD,0,&pcClipper,NULL);
	if( FAILED( hr ) )
		return hr;
	...
	IDirectDrawSurface7_SetClipper( g_pddsPrimary, pcClipper );
	...
	IDirectDraw7_QueryInterface( g_pDD, &IID_IDirect3D7, (VOID**)&g_pD3D);
    if( FAILED( hr) )
		return FALSE;
	...
	ddsd.dwSize = sizeof(DDSURFACEDESC2);
	IDirectDraw7_GetDisplayMode(g_pDD,&ddsd);
	...
	hr = IDirect3D7_CreateDevice( g_pD3D, &IID_IDirect3DHALDevice, g_pddsBackBuffer, &g_pd3dDevice );
//    hr = g_pD3D->CreateDevice( IID_IDirect3DHALDevice, g_pddsBackBuffer,
	...
	if( FAILED( hr ) )
	{
		...
		// software rasterizer.
		hr = IDirect3D7_CreateDevice( g_pD3D, &IID_IDirect3DRGBDevice, g_pddsBackBuffer, &g_pd3dDevice );
	...
	g_vp.dwX=0;
    g_vp.dwY=0;            /* Viewport Top left */
    g_vp.dwWidth=dwRenderWidth;
    g_vp.dwHeight=dwRenderHeight;       /* Viewport Dimensions */
    g_vp.dvMinZ=0.0f;         /* Min/max of clip Volume */
    g_vp.dvMaxZ=1.0f;
	hr = IDirect3DDevice7_SetViewport( g_pd3dDevice, &g_vp );
	if( FAILED( hr ) )
		return FALSE;
...
//syq 2001.1.15
BOOL CALLBACK TimerWaitDialogProc(
...
void VID_InitDIB (HINSTANCE hInstance)
{
	...
	//avoid by syq 2000.10.31 //no avoid 2000.1.12
...
HRESULT ShowFrame()
{
    ...
    return IDirectDrawSurface7_Blt(g_pddsPrimary,&g_rcScreenRect, g_pddsBackBuffer, 
                               NULL, DDBLT_WAIT, NULL );
}

void InitRenderState()
{
    D3DMATRIX matWorld, matProj , matView;
	D3DMATERIAL7 mtrl;
	FLOAT fAspect;

	// set texture
    IDirect3DDevice7_SetTextureStageState( g_pd3dDevice, 0, D3DTSS_COLORARG1, D3DTA_TEXTURE );
	IDirect3DDevice7_SetTextureStageState( g_pd3dDevice, 0, D3DTSS_COLORARG2, D3DTA_DIFFUSE );
	IDirect3DDevice7_SetTextureStageState( g_pd3dDevice, 0, D3DTSS_COLOROP,   /*D3DTOP_MODULATE*/D3DTOP_SELECTARG1 );
	IDirect3DDevice7_SetTextureStageState( g_pd3dDevice, 0, D3DTSS_MINFILTER, D3DTFN_LINEAR );
	IDirect3DDevice7_SetTextureStageState( g_pd3dDevice, 0, D3DTSS_MAGFILTER, D3DTFG_LINEAR );

	IDirect3DDevice7_SetTextureStageState( g_pd3dDevice, 1, D3DTSS_TEXCOORDINDEX, 1 );
	IDirect3DDevice7_SetTextureStageState( g_pd3dDevice, 1, D3DTSS_MINFILTER, D3DTFN_LINEAR );
	IDirect3DDevice7_SetTextureStageState( g_pd3dDevice, 1, D3DTSS_MAGFILTER, D3DTFG_LINEAR );

    // Set miscellaneous render states
    IDirect3DDevice7_SetRenderState( g_pd3dDevice, D3DRENDERSTATE_DITHERENABLE,   FALSE );
    IDirect3DDevice7_SetRenderState( g_pd3dDevice, D3DRENDERSTATE_SPECULARENABLE, FALSE );
    IDirect3DDevice7_SetRenderState( g_pd3dDevice, D3DRENDERSTATE_ZENABLE,        TRUE );
	IDirect3DDevice7_SetRenderState( g_pd3dDevice, D3DRENDERSTATE_AMBIENT,        D3DRGBA(1.0f,1.0f,1.0f,1.0f) );
	IDirect3DDevice7_SetRenderState( g_pd3dDevice, D3DRENDERSTATE_LIGHTING, FALSE );

	//set the world matrix
	D3DUtil_SetIdentityMatrix(&matWorld);
	IDirect3DDevice7_SetTransform( g_pd3dDevice, D3DTRANSFORMSTATE_WORLD, &matWorld );
    // Set the projection matrix
	fAspect=(FLOAT)(g_rcScreenRect.right - g_rcScreenRect.left)/(g_rcScreenRect.bottom - g_rcScreenRect.top);
    D3DUtil_SetProjectionMatrix( &matProj, g_PI/3.0f, fAspect, 1.0f, 999999.0f );
...
HRESULT CreateZBuffer( GUID* pDeviceGUID )
{
	...
	IDirect3DDevice7_GetCaps( g_pd3dDevice, &ddDesc );
    if( ddDesc.dpcTriCaps.dwRasterCaps & D3DPRASTERCAPS_ZBUFFERLESSHSR )
        return S_OK;

    // Get z-buffer dimensions from the render target
    ddsd.dwSize = sizeof(ddsd);
	IDirectDrawSurface7_GetSurfaceDesc( g_pddsBackBuffer, &ddsd );
    ...
    if( IsEqualIID( pDeviceGUID, &IID_IDirect3DHALDevice) )
        ...
        dwDeviceMemType = DDSCAPS_SYSTEMMEMORY;
    ...
    IDirect3D7_EnumZBufferFormats( g_pD3D, pDeviceGUID, EnumZBufferFormatsCallback,
    ...
    if( FAILED( hr = IDirectDraw7_CreateSurface( g_pDD, &ddsd, &pddsZBuffer, NULL ) ) )
    {
		MessageBox(NULL,GetErrorString(hr),"",MB_OK);
    ...
    if( FAILED( IDirectDrawSurface7_AddAttachedSurface( g_pddsBackBuffer, pddsZBuffer ) ) )
    {
    ...
    if( FAILED( IDirect3DDevice7_SetRenderTarget( g_pd3dDevice, g_pddsBackBuffer, 0L ) ) )
    {
...
)
{
	...
	case WM_INITDIALOG:
		...
		PostMessage(GetDlgItem(hwndDlg,IDC_PROGRESS),PBM_SETRANGE,0,MAKELPARAM(0, DELAY_TIME*100)); 
		SetTimer(hwndDlg,TIMER_ID,100,NULL);
		nCount=0;
		return TRUE;
		break;
	case WM_DESTROY:
	...
	case WM_TIMER:
		if (TIMER_ID==wParam) {
			PostMessage(GetDlgItem(hwndDlg,IDC_PROGRESS),PBM_STEPIT,0,0);
			nCount++;
			if (nCount>DELAY_TIME*10)
				EndDialog(hwndDlg,0);
		}
		break;
	case WM_COMMAND:
		...
		case IDC_BUTTON2:{
			char szWWW[MAX_PATH];
			GetDlgItemText(hwndDlg,wParam,szWWW,MAX_PATH);
			ShellExecute(NULL,"open",szWWW,NULL,NULL,SW_SHOWDEFAULT);	
						 }
			break;
		}
		return TRUE;
...
void ViewWWW() //syq 2001.1.15
{/*
	DialogBox( global_hInstance,  // handle to application instance
			MAKEINTRESOURCE(IDD_TIMERWAIT),
			mainwindow,
			TimerWaitDialogProc);
			*/
}