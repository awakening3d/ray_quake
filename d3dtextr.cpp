...
public:
    ...
    HRESULT Restore( LPDIRECT3DDEVICE7 pd3dDevice, UINT* pData=NULL );
    HRESULT CopyBitmapToSurface();
    HRESULT CopyRGBADataToSurface( UINT* pData=NULL );
...
//-----------------------------------------------------------------------------
HRESULT TextureContainer::Restore( LPDIRECT3DDEVICE7 pd3dDevice, UINT* pData/*=NULL*/)
{
	...
	if (pData)
		return CopyRGBADataToSurface(pData);
...
//-----------------------------------------------------------------------------
HRESULT TextureContainer::CopyRGBADataToSurface(UINT* pData/*=NULL*/)
{
    // Get a DDraw object to create a temporary surface
    LPDIRECTDRAW7 pDD;
    m_pddsSurface->GetDDInterface( (VOID**)&pDD );
...
*/
    ...
    for( DWORD y=0; y<ddsd.dwHeight; y++ )
    {
        ...
        for( DWORD x=0; x<ddsd.dwWidth; x++ )
        {
            DWORD dwPixel;
			if (pData)
				dwPixel = pData[y*ddsd.dwWidth+x];
			else
				dwPixel = m_pRGBAData[y*ddsd.dwWidth+x];
...
//       using the new device.
//-----------------------------------------------------------------------------
HRESULT D3DTextr_Restore( TCHAR* strName, LPDIRECT3DDEVICE7 pd3dDevice, UINT* pData/*=NULL*/)
{
    ...
    return ptcTexture->Restore( pd3dDevice, pData);
}
...
HRESULT D3DTextr_RestoreAllTextures( LPDIRECT3DDEVICE7 pd3dDevice )
{
    ...
    while( ptcTexture )
    {
        D3DTextr_Restore( ptcTexture->m_strName, pd3dDevice, NULL);
        ptcTexture = ptcTexture->m_pNext;
    }
