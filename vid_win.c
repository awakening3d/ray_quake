...
int VID_Suspend (MGLDC *dc,M_int32 flags)
{
...
//we don't want VESA 1.X drivers		MGL_registerDriver(MGL_SVGA8NAME,SVGA8_driver);
		MGL_registerDriver(MGL_SVGA8NAME,LINEAR8_driver);

		if (!COM_CheckParm ("-novbeaf"))
			MGL_registerDriver(MGL_SVGA8NAME,ACCEL8_driver);
	}

	if (useDirectDraw)
	{
		MGL_registerDriver(MGL_SVGA8NAME,DDRAW8_driver);
	}
}
...
#undef MGL_createDisplayDC
//	if ((dc = MGL_createDisplayDC(npages)) == NULL) //888
	if ((dc = MGL_createDisplayDC(0,2,60)) == NULL)
		return NULL;
				...
				while (rects)
				{
					if (vid_stretched)
					{
						MGL_stretchBltCoord(mgldc, memdc,
									...
									(rects->y + rects->height) << 1,MGL_REPLACE_MODE);
					}
					else
					{
			...
			while (rects)
			{
				if (vid_stretched)
				{
					MGL_stretchBltCoord(windc,dibdc,
						...
						rects->x << 1, rects->y << 1,
						(rects->x + rects->width) << 1,
						(rects->y + rects->height) << 1,MGL_REPLACE_MODE);//888
				}
				else
				{
