...
void Draw_StretchRaw(int x, int y, int w, int h, int cols, int rows, byte *data, qboolean rgba)
{
	...
	if (rgba) {
		memcpy(image32, data, sizeof(unsigned)*cols*rows);
		glTexImage2D(GL_TEXTURE_2D, 0, gl_solid_format, cols, rows, 0, GL_RGBA, GL_UNSIGNED_BYTE, image32);
		t = 1;
		goto endtex;
	}
		...
		for (i=0, glt=gltextures ; i<numgltextures ; i++, glt++)
		{
			if (!strcmp (identifier, glt->identifier))
			{
				if (width != glt->width || height != glt->height)
					Sys_Error ("GL_LoadTexture: cache mismanumgltexturestch");
				return gltextures[i].texnum;
			}
		}
	}
