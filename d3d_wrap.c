// d3d_warp.cpp -- sky and water polygons
...
================
*/
void D3D_SubdivideSurface (msurface_t *fa)
{
	vec3_t		verts[64];
	int			numverts;
	int			i;
...
void R_LoadSkys (void)
{
	...
	SKY_TEX=texture_extension_number;
	for (i=0 ; i<6 ; i++)
	{
//		GL_Bind (SKY_TEX + i);
...
//		LoadPCX (f);
		...
		D3D_LoadTexture(name,256,256,targa_rgba,FALSE,FALSE);
...
/*	for (i=0 ; i<6 ; i++)
		...
		D3D_Bind (SKY_TEX+skytexorder[i]);
		*/
/*		glBegin (GL_QUADS);
...
//	transpix = (255<<0) + (b/(128*128)<<8) + (g/(128*128)<<16) +(r/(128*128)<<24);

//	if (!solidskytexture)
//		solidskytexture = texture_extension_number++;

	solidskytexture = D3D_LoadTexture32("skysolidskytexture",128,128,trans,FALSE,FALSE);
...
*/
	...
	alphaskytexture = D3D_LoadTexture32("alphaskytexture",128,128,trans,FALSE,TRUE);
/*	if (!alphaskytexture)
